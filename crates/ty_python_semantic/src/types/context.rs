        ...
        if let Some(hint) = self.class_statement_hints {
            match hint {
                ...
                ClassStatementHint::MetaclassConstructor => {
                    diag.info(
                        "A `class` statement implicitly invokes \
                        the constructor of the class's metaclass",
                    );
                    diag.info("See https://docs.python.org/3/reference/datamodel.html#metaclasses");
                }
