...
use std::sync::Mutex;
use thiserror::Error;
use ty_python_semantic::{SuppressFix, is_unused_ignore_comment_lint, suppress_all};
...
fn fix_all(
    ...
    for file in completed {
        ...
        if let Some(fixed) = source_texts.uncommited(file.file) {
...
impl SourceTexts {
    ...
    /// Returns any uncommited changes (staged or unstaged).
    ///
    /// Returns `None` if there are no uncommited changes.
    fn uncommited(&self, file: File) -> Option<&SourceText> {
    ...
    fn reset_unstaged(&mut self, db: &mut dyn Db, file: File) {
        ...
        let source = if let Some(staged) = self.staged_changes.get(&file) {
            ...
            // File was never overriden, nothing to do
            return;
        };
            ...
            for (mut file, applied_fixes) in changes {
                ...
                scope.spawn(move |_| {
                    ...
                    let result = if parsed.has_syntax_errors() {
                        ...
                        let diagnostics = db.project().check_file(db, file.file);
